...
`,Wt=g`
  ${Gt}
`;function j(i,t,e){if(typeof window!="undefined")try{window.customElements.define(i,t,e)}catch(s){if(s instanceof DOMException){const n="Failed to execute 'define' on 'CustomElementRegistry'",r="has already been used with this registry";if(s.message.includes(n)&&s.message.includes(r))return}throw s}}const qt=g`
:root {
...
`}));const Ae={get(i){return ht.get(i)}},be="cta-icon",Ee=i=>Ae.get(i);class O extends b{constructor(){super(...arguments),this.name="",this.inline=!1}getIcon(){return Ee(this.name)}render(){const t=this.getIcon();return x`
    ...
    `}}return O.tagName=be,O.styles=g`
    :host {
      display: flex;
    }
    :host([inline]) {
